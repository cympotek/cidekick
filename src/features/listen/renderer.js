...
const CIDEKICK_SYSTEM_PROMPT = `<core_identity>
You are Cidekick, developed and created by Cidekick, and you are the user's live-meeting co-pilot.
</core_identity>
...
window.cidekick = window.cidekick || {};
...
function cidekickElement() {
    return document.getElementById('cidekick');
}
...
async function initializeopenai(profile = 'interview', language = 'en') {
    ...
    try {
        ...
        if (success) {
            ...
            const appElement = cidekickElement();
...
ipcRenderer.on('stt-update', (event, data) => {
    ...
    if (cidekick.e() && typeof cidekick.e().updateRealtimeTranscription === 'function') {
        cidekick.e().updateRealtimeTranscription({
            speaker,
            text,
            isFinal,
            isPartial,
...
ipcRenderer.on('update-structured-data', (_, structuredData) => {
    ...
    window.cidekick.structuredData = structuredData;
    window.cidekick.setStructuredData(structuredData);
});
window.cidekick.structuredData = {
    summary: [],
    topic: { header: '', bullets: [] },
    actions: [],
};
window.cidekick.setStructuredData = data => {
    window.cidekick.structuredData = data;
    cidekick.e()?.updateStructuredData?.(data);
};
...
ipcRenderer.on('session-state-changed', (_event, { isActive }) => {
    if (!isActive) {
        ...
        window.cidekick.setStructuredData(blankData);
    }
});
