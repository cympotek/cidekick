...
function safeSend(window, channel, ...args) {
    try {
        ...
        console.error(`Error sending IPC message '${channel}':`, error.message);
    }
    return false;
}
...
function createFeatureWindows(header) {
    ...
    ask.on('blur',()=>safeSend(ask, 'window-blur'));
    ...
    if (header.isVisible()) {
        ...
        windowPool.forEach((win, name) => {
            if (win.isVisible()) {
                ...
                if (name !== 'header') {
                    safeSend(win, 'window-hide-animation');
                    setTimeout(() => {
                        if (!win.isDestroyed()) {
                            win.hide();
                        }
        ...
        movementManager.showFromEdge(() => {
            lastVisibleWindows.forEach(name => {
                ...
                if (win && !win.isDestroyed()) {
                    win.show();
                    safeSend(win, 'window-show-animation');
                }
            });
...
function ensureDataDirectories() {
    ...
    const cidekickDir = path.join(homeDir, '.cidekick');
    const dataDir = path.join(cidekickDir, 'data');
    const imageDir = path.join(dataDir, 'image');
    const audioDir = path.join(dataDir, 'audio');

    [cidekickDir, dataDir, imageDir, audioDir].forEach(dir => {
...
function createWindows() {
            ...
            if (askWindow.isVisible()) {
                try {
                    const hasResponse = await askWindow.webContents.executeJavaScript(`
                        (() => {
                            try {
                                // CidekickApp의 Shadow DOM 내부로 접근
                                const cidekickApp = document.querySelector('cidekick-app');
                                if (!cidekickApp || !cidekickApp.shadowRoot) {
                                    console.log('CidekickApp not found');
                                    return false;
                                }
                                
                                // CidekickApp의 shadowRoot 내부에서 ask-view 찾기
                                const askView = cidekickApp.shadowRoot.querySelector('ask-view');
                                if (!askView) {
                                    console.log('AskView not found in CidekickApp shadow DOM');
                                    return false;
                                }
                ...
                askWindow.show();
                updateLayout();
                safeSend(askWindow, 'window-show-animation');
                safeSend(askWindow, 'window-did-show');
            }
        } else {
            const windowToToggle = windowPool.get(featureName);
            ...
            if (windowToToggle) {
                ...
                if (windowToToggle.isVisible()) {
                    if (featureName === 'settings') {
                        safeSend(windowToToggle, 'settings-window-hide-animation');
                    } else {
                        safeSend(windowToToggle, 'window-hide-animation');
                    }
                    ...
                    try {
                        ...
                        if (featureName === 'listen') {
                            safeSend(windowToToggle, 'start-listening-session');
                        }
                        safeSend(windowToToggle, 'window-show-animation');
                    } catch (e) {
    ...
    ipcMain.handle('send-question-to-ask', (event, question) => {
        ...
        if (askWindow && !askWindow.isDestroyed()) {
            ...
            safeSend(askWindow, 'receive-question-from-assistant', question);
            return { success: true };
        } else {
...
function setupIpcHandlers(openaiSessionRef) {
    ...
    ipcMain.on('ask-response-chunk', (event, { token }) => {
        ...
        if (askWindow && !askWindow.isDestroyed()) {
            ...
            safeSend(askWindow, 'ask-response-chunk', { token });
        }
    });
    ...
    ipcMain.on('ask-response-stream-end', () => {
        const askWindow = windowPool.get('ask');
        if (askWindow && !askWindow.isDestroyed()) {
            safeSend(askWindow, 'ask-response-stream-end');
        }
    });
    ...
    ipcMain.handle('message-sending', async event => {
        ...
        if (askWindow && !askWindow.isDestroyed()) {
            ...
            safeSend(askWindow, 'hide-text-input');
            return { success: true };
        }
    ...
    ipcMain.handle('open-login-page', () => {
        const webUrl = process.env.CIDEKICK_WEB_URL || 'http://localhost:3000';
    ...
    ipcMain.handle('force-close-window', (event, windowName) => {
        ...
        if (window && !window.isDestroyed()) {
            ...
            safeSend(window, 'window-hide-animation');
