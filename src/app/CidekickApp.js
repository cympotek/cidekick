...
export class CidekickApp extends LitElement {
    static styles = css`
        :host {
            display: block;
            width: 100%;
    ...
    async handleListenClick() {
        ...
        if (window.cidekick) {
            await window.cidekick.initializeopenai(this.selectedProfile, this.selectedLanguage);
            window.cidekick.startCapture(this.selectedScreenshotInterval, this.selectedImageQuality);
        }
    ...
    async handleSendText(message) {
        if (window.cidekick) {
            const result = await window.cidekick.sendTextMessage(message);
    ...
    //     console.log('📝 CidekickApp updateOutline:', outline);
    //     this.outlines = [...outline];
    //     this.requestUpdate();
    // }

    // updateAnalysisRequests(requests) {
    //     console.log('📝 CidekickApp updateAnalysisRequests:', requests);
    //     this.analysisRequests = [...requests];
    //     this.requestUpdate();
    // }

    updateStructuredData(data) {
        console.log('📝 CidekickApp updateStructuredData:', data);
...
customElements.define('cidekick-app', CidekickApp);
