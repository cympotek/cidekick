...
app.setAsDefaultProtocolClient('cidekick');
...
function setupGeneralIpcHandlers() {
    ...
    ipcMain.handle('get-api-url', () => {
        return process.env.CIDEKICK_API_URL || 'http://localhost:9001';
    });

    ipcMain.handle('get-web-url', () => {
        return process.env.CIDEKICK_WEB_URL || 'http://localhost:3000';
    });

    ipcMain.on('get-api-url-sync', (event) => {
        event.returnValue = process.env.CIDEKICK_API_URL || 'http://localhost:9001';
    });
...
async function startWebStack() {
  ...
  process.env.CIDEKICK_API_PORT = apiPort.toString();
  process.env.CIDEKICK_API_URL = `http://localhost:${apiPort}`;
  process.env.CIDEKICK_WEB_PORT = frontendPort.toString();
  process.env.CIDEKICK_WEB_URL = `http://localhost:${frontendPort}`;

  console.log(`🌍 Environment variables set:`, {
    CIDEKICK_API_URL: process.env.CIDEKICK_API_URL,
    CIDEKICK_WEB_URL: process.env.CIDEKICK_WEB_URL
  });

  const createBackendApp = require('../cidekick_web/backend_node');
  const nodeApi = createBackendApp();

  const staticDir = app.isPackaged
    ? path.join(process.resourcesPath, 'out')
    : path.join(__dirname, '..', 'cidekick_web', 'out');
  ...
  if (!fs.existsSync(staticDir)) {
    ...
    console.error(`Please run 'npm run build' inside the 'cidekick_web' directory first.`);
    console.error(`============================================================`);
    app.quit();
    return;
  }
